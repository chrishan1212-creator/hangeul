...
| 🎮 찾기 게임 | 소리를 듣고 맞는 글자를 골라요 (아기공룡이 문제를 냅니다) |
...
- **먹을 때마다 공룡이 조금씩 커집니다** (2회차, 3회차로 갈수록 더 커져요)
- 음식은 매번 새로 정해집니다. 초식공룡(🦕)은 풀·나뭇잎, 육식공룡(🦖)은
  고기·닭다리를 좋아하지만, **아이스크림·사탕·과자·빵·밥·국수·과일** 같은
  간식도 나와서 "이번엔 뭐가 나올까?" 기대하게 됩니다
- 다 먹으면 공룡이 음식 이름을 말해줘요 (*"냠냠! 도넛이 정말 맛있어. 고마워!"*)
...
src/
  ...
  lib/
    ...
    dino.ts            # 공룡 종류, 음식, 성장 크기
    dinoLines.ts       # 단어 분류별 공룡 대사
